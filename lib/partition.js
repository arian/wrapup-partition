...
var Partition = prime({
    ...
    modulesByFileID: function(_path){
        var _modules = this.modulesByID()
        var modules = {}
        forOwn(_modules, function(module){
            modules[module.getModuleID(_path)] = module
        })
        return modules
    },
    ...
    output: function(callback, defineAST, wrapperAST, mainAST){
        ...
        var modules = this.modulesByFileID(this._path)
        ...
        forOwn(map, function(files, out){
            var modulesInFile = map[out] = []
            files.forEach(function(file, i){
                var module = modules[file]
                if (module){
                    added[module.full] = true
                    modulesInFile.push(module)
                } else {
    ...
    addModulesToAST: function(wrapper, modules, defineAST){
        ...
        modules.forEach(function(module){
            ...
            Output.replaceRequire(ast, module, function(dep){
                return dep.getModuleFile(self._path)
            })
            ...
            // change module ID
            args[0].value = module.getModuleID(self._path)

            // add dependencies to the dependency array
            var deps = args[1].elements
            module.deps.forEach(function(dep){
                if (dep) deps.push({type: "Literal", value: dep.getModuleID(self._path)})
            })
